...
for i in images:
    ...
    img_bw_blurred = cv2.GaussianBlur(img_bw, (11, 11), 0)
    img_bw_diff11 = cv2.absdiff(img_bw, img_bw_blurred)
    # show_image(img_bw_blurred, "diff11")

    img_bw_blurred = cv2.GaussianBlur(img_bw, (31, 31), 0)
    img_bw_diff31 = cv2.absdiff(img_bw, img_bw_blurred)
    # show_image(img_bw_blurred, "diff31")
    
    img_bw_blurred = cv2.GaussianBlur(img_bw, (51, 51), 0)
    img_bw_diff = cv2.absdiff(img_bw, img_bw_blurred)
    # show_image(img_bw_blurred, "diff51")

    show_image(np.hstack((img_bw_diff11, img_bw_diff31, img_bw_diff)), "diff", (600*3, 800))
    ...
    # img_bw_inv = cv2.bitwise_not(img_bw_diff)
    img_bw_close = cv2.morphologyEx(img_bw_diff, cv2.MORPH_CLOSE, make_kernel(5), iterations=1)

    img_thresh = cv2.threshold(img_bw_diff, 30, 255, cv2.THRESH_BINARY)[1]
    ...
    lines = cv2.HoughLinesP(img_thresh, 1, np.pi/180, 400, minLineLength=30, maxLineGap=10)
